{{-- resources/views/your-view-folder/edit-role.blade.php --}}
            ...
            <div class="mb-3">
                ...
                <select multiple class="form-select" name="permissions[]" id="permissions" size="5" aria-label="permissions">
                    @foreach($permissions as $permission)
                        <option value="{{ $permission->id }}" {{ in_array($permission->id, old('permissions') ?: $role->permissions->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $permission->name }}</option>
                    @endforeach
                </select>
                @error('permissions')
                <div class="text-sm text-danger">{{ $message }}</div>
